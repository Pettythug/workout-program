...
function gymlog_recalculateBestForExercise(exerciseName) {
  SpreadsheetApp.flush(); // Force Google Sheets to execute pending appendRow/deleteRow commands before reading
  ...
  const entries = histData.filter(r => String(r[2]).trim() === targetName).map(r => ({
  ...
  if (lastBestRow > 1) {
    ...
    for (let i = 0; i < names.length; i++) {
      if (String(names[i][0]).trim() === targetName) { rowIndex = i + 2; break; }
    }
  }
