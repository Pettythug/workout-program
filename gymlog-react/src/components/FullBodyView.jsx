...
export default function FullBodyView() {
    ...
    useEffect(() => {
        localStorage.setItem('gymlog_fullBody_session_accessories', JSON.stringify(accessoriesList));
    }, [accessoriesList]);
    ...
    const plannedExercises = useMemo(() => {
        ...
        const daySwaps = (fullBodySwaps && fullBodySwaps[fullBodyWorkoutDay]) || {};
    ...
    }, [groupedExercises, fullBodyWorkoutDay, fullBodySwaps, activeLocation]);
    ...
    const startNextWorkout = () => {
        ...
        plannedExercises.forEach(ex => {
            if (ex && ex.rotationKey) {
                let currentIdx = parseInt(localStorage.getItem('gymlog_fullBody_rotation_' + ex.rotationKey) || '0', 10);
                if (isNaN(currentIdx) || currentIdx < 0) currentIdx = 0;
                const nextIdx = currentIdx + 1;
                localStorage.setItem('gymlog_fullBody_rotation_' + ex.rotationKey, nextIdx.toString());
                console.log(`[Full Body Rotation Audit] Incremented key: ${ex.rotationKey} | Old: ${currentIdx} | New: ${nextIdx}`);
            }
        });
        ...
        localStorage.setItem('gymlog_fullBody_complete', 'false');
        setView('tracker');

        // 5. Clean up accessories
        localStorage.removeItem('gymlog_fullBody_session_accessories');
        setAccessoriesList([]);
    };
            ...
            <div className="header" style={{ margin: '-16px -16px 16px', position: 'sticky', top: 0, zIndex: 100 }}>
                <div>
                    ...
                    <div style={{ fontSize: 10, color: 'var(--muted)', letterSpacing: 2, fontFamily: 'var(--mono)', marginTop: 3 }}>
                        #{fullBodyWorkoutDay} | 8 Core Exercises
                    </div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                    <div 
                        style={{ background: 'rgba(249, 115, 22, 0.1)', color: 'var(--accent)', padding: '4px 10px', borderRadius: 20, fontSize: 11, fontFamily: 'var(--mono)', fontWeight: 500, display: 'flex', alignItems: 'center' }}
                    >
                        8-SPLIT
                    </div>
                ...
                if (isWorkoutComplete) {
                    return (
                        <div style={{ textAlign: 'center', padding: 40, color: 'var(--success)', background: '#111', borderRadius: 12, border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
                            ...
                            <h2 style={{ fontSize: 22, fontWeight: 'bold' }}>Full Body Workout Complete!</h2>
                            <p style={{ color: 'var(--muted)', fontSize: 13 }}>Incredible work finishing all 8 core exercise categories.</p>
                ...
                if (activeIdx >= plannedExercises.length) {
                    return (
                        <>
                            <div style={{ textAlign: 'center', padding: 40, color: 'var(--success)', background: '#111', borderRadius: 12, border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 16 }}>
                                <div style={{ fontSize: 40 }}>✅</div>
                                <h2 style={{ fontSize: 22, fontWeight: 'bold' }}>All 8 Exercises Done!</h2>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        <div className="info-bar" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 0 }}>
                            <div className="info-item" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 12, textAlign: 'center' }}>
                                ...
                                <div className="value" style={{ fontSize: 18, fontWeight: 600, color: 'var(--accent)', fontFamily: 'var(--mono)' }}>{fullBodyWorkoutDay}</div>
                            </div>
                            <div className="info-item" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 12, textAlign: 'center' }}>
                                <div className="label" style={{ fontSize: 10, color: 'var(--muted)', textTransform: 'uppercase', fontFamily: 'var(--mono)' }}>Reps</div>
                                <div className="value" style={{ fontSize: 18, fontWeight: 600, color: 'var(--accent)', fontFamily: 'var(--mono)' }}>{getRepRange(fullBodyWorkoutDay)}</div>
                            </div>
                            <div className="info-item" style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 12, textAlign: 'center' }}>
